...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        url_to_code={}
        with open('/'.join(index_filename.split('/')[:-1])+f'/data{args.abs}.jsonl') as f:
            for line in f:
                line=line.strip()
                js=json.loads(line)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1']>best_f1:
                        ...
                        checkpoint_prefix = f'checkpoint-{args.dataset}-{args.split}-{args.abs}'
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    for step, batch in enumerate(bar):
        ...
        nb_eval_steps += 1
        bar.set_description(f"loss: {eval_loss}")
    ...
    with open(f'./result_{args.dataset}_{args.split}_{args.abs}.log', 'a') as f:
        f.write(args.train_data_file)
        f.write("\t")
        f.write(args.eval_data_file)
        f.write("\n"+str(result))
        f.write("\n")
...
def main():
    ...
    parser.add_argument("--abs", default="0", type=str)   
    parser.add_argument("--dataset", default="gcj", type=str)  
    ...
    results = {}
    if args.do_eval:
        checkpoint_prefix = f'checkpoint-{args.dataset}-{args.split}-{args.abs}/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
