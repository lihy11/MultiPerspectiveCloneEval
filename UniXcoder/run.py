        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if len(losses) % args.save_steps == 0:
                ...
                if results['eval_f1'] > best_f1:
                    ...
                    checkpoint_prefix = f'checkpoint-{args.dataset}'
                    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = model.module if hasattr(model, 'module') else model
                    output_dir = os.path.join(output_dir, f'{args.split}_{args.abs}.bin')
...
def evaluate(args, model, tokenizer, data_file):
    ...
    with open(f'./result_{args.dataset}_{args.split}_{args.abs}.log', 'a') as f:
        f.write(f'{args.train_data_file}\t{args.eval_data_file}\t{len(eval_dataloader)}\n')
        f.write(str(result))
        f.write("\n")
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='gcj')
    ...
    if args.retrain:
        train_dataset = TextDataset(tokenizer, args, args.train_data_file)
        checkpoint_prefix = f'checkpoint-{args.dataset}/{args.split}_{args.abs}.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
